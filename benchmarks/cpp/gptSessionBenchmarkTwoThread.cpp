...
std::string engineFilename(GptJsonConfig json, WorldConfig const& worldConfig, std::string const& model, int rank = -1)
{
    TLLM_CHECK_WITH_INFO(json.getTensorParallelism() == worldConfig.getTensorParallelism(), "tensor parallelism mismatch");
    TLLM_CHECK_WITH_INFO(
        json.getPipelineParallelism() == worldConfig.getPipelineParallelism(), "pipeline parallelism mismatch");
    auto pp = worldConfig.isPipelineParallel() ? "_pp" + std::to_string(worldConfig.getPipelineParallelism()) : "";
    if (json.getVersion() == std::string("none"))
    {
        return model + "_" + json.getPrecision() + "_tp" + std::to_string(worldConfig.getTensorParallelism()) + pp + "_rank"
            + std::to_string(rank) + ".engine";
    }
    else
    {
...
void benchmarkGptSession(std::string const modelName, std::filesystem::path const dataPath,
    std::vector<int>  batchSizes, const int beamWidth, std::vector<std::vector<int>>  inOutLen,
    std::shared_ptr<nvinfer1::ILogger> const logger, const int warmUp,const int numRuns,const int duration,
    GptSession::Config sessionConfig, bool cudaGraphMode, bool printAllLogits, bool disableForceMaxTokens,int rank)
{
    ...
    auto const worldConfig = WorldConfig::twothread(deviceCount, json.getTensorParallelism(), json.getPipelineParallelism(), rank);
    auto const enginePath = dataPath / engineFilename(json, worldConfig, modelNameHyphen, rank);//("llama_float16_tp2_rank" + std::to_string(rank) + ".engine");//
    ...
    for (auto inOut : inOutLen)
    {
        ...
        GptSession session{sessionConfig, modelConfig, worldConfig, enginePath.string(),logger,rank};
...
int main(int argc, char* argv[])
{
    ...
    std::string dataPath = result["engine_dir"].as<std::string>();
    initTrtLlmPlugins(logger.get());

    auto start = std::chrono::high_resolution_clock::now();
    try
    {
        // sem_init(&sema_rank0, 0, 0);
        // sem_init(&sema_rank1, 0, 0);
        sem_init(&semaphore, 0, 0);
        std::thread thread1(benchmarkGptSession,result["model"].as<std::string>(), result["engine_dir"].as<std::string>(), batchSizes,
                beamWidth, inOutLen, logger, result["warm_up"].as<int>(), result["num_runs"].as<int>(),
                result["duration"].as<int>(), sessionConfig, enableCudaGraph, printAllLogits, disableForceMaxTokens,0);
        std::thread thread2(benchmarkGptSession,result["model"].as<std::string>(), result["engine_dir"].as<std::string>(), batchSizes,
                beamWidth, inOutLen, logger, result["warm_up"].as<int>(), result["num_runs"].as<int>(),
                result["duration"].as<int>(), sessionConfig, enableCudaGraph, printAllLogits, disableForceMaxTokens,1);
        sleep(30);
        sem_post(&semaphore);
        sem_post(&semaphore); 
        thread1.join();
