...
def parse_nmt_config(config, component, args):
    ...
    args.hidden_size = config.getint(component,
                                     f'{component}_embed_dim')  # fairseq naming
    ...
    args.n_positions = config.getint(component,
                                     'max_source_positions')  # fairseq naming
    ...
    args.layernorm_type = layernorm_type_map[config.get(component,
                                                        'layernorm_type',
                                                        fallback='LayerNorm')]
    args.hidden_act = config.get(component, 'activation_fn')  # fairseq naming
...
def load_from_binary_nmt(tllm_model: Union[DecoderModel, EncoderModel],
                         dir_path,
                         args,
    ...
    if mapping.is_first_pp_rank():
        wte = fromfile(f'{component}.embed_tokens.weight',
        ...
        wpe = fromfile(f'{component}.embed_positions.weight',
    ...
    for local_idx, global_idx in enumerate(
        ...
        layer_prefix = f'{component}.layers.{global_idx}'
    ...
    if mapping.is_last_pp_rank():
        if tllm_model.has_model_final_layernorm:  # mBART true BART false NMT false
